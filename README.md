...
```
grep "failure" transaction_data_daily_event_log_20190129.dat
```

Upon review, we would like to only view failures with error code SYSOFFLINE or WEAKSIGNAL.

```
ggrep "failure" transaction_data_daily_event_log_20190129.dat | grep "SYSOFFLINE\|WEAKSIGNAL"
```
...
```
ggrep -w '\.edu' users.csv > academia_users.txt.
```
